...
| attachment                           | id, task_id, project_id, user_id, r2_key, filename, size, content_type, created_at                                                       |
...
- Project, task, and attachment schemas are implemented under `src/db/schema/`.
  AI usage schema belongs to F-013.
...
- **Placement:** the production Worker runs next to its D1 primary through a placement hint, not in each visitor's nearest location ([ADR-0011](decisions/0011-placement-near-d1.md)).
